...
tot_to_remove=`cat $processing_path/sh/to_remove_issue_identifiers.txt | wc -l`

if (($tot_to_remove < 2000)); then
    for pid in `cat $processing_path/sh/to_remove_issue_identifiers.txt`;
    do
      collection=${pid:0:3}
      pid=${pid:3:17}
      durl="http://"$scielo_data_url"/api/v1/issue/delete/?code="$pid"&collection="$collection"&admintoken="$admintoken
      curl -X DELETE $durl
    done
else
